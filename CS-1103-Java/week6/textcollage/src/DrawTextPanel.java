...
//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
    ...
    public JMenuBar getMenuBar() {
        if (menuBar == null) {
            ...
            JMenuItem formatAllTextColor = new JMenuItem("Format All Text Color ...");
            formatAllTextColor.addActionListener(menuHandler);
            optionsMenu.add(formatAllTextColor);
    ...
    private void doMenuCommand(String command) {
        if (command.equals("Save...")) { // save all the string info to a file
            ...
            theString.clear();
            ;   // Remove the ONLY string from the canvas.
            undoMenuItem.setEnabled(false);
            canvas.repaint();
            ...
            if (c != null)
                currentTextColor = c;
        } else if (command.equals("Format All Text Color ...")) {
            Color c = JColorChooser.showDialog(this, "Select Text Color", new Color(65280));
            if (c != null) {
                for (DrawTextItem item : theString) {
                    item.setTextColor(c);
                }
    ...
    private void saveStringToFile() {
        File file = fileChooser.getOutputFile(this, "Please select an output file to save", "output.txt");
        if (file == null) {
            return;  // user cancelled
        }
        try {
            PrintWriter writer = new PrintWriter(file);
            writer.println(canvas.getBackground().getRGB());
            ...
            for (DrawTextItem item : theString) {
                writer.println(
                        item.getString() + "|"
                                + item.getX() + "|"
                                + item.getY() + "|"
                                + item.getTextColor().getRGB()
                );
            }
            writer.close();
            JOptionPane.showMessageDialog(canvas, "Output file has been successfully saved to " + file);

        } catch (Exception e) {
            JOptionPane.showMessageDialog(canvas, "Error saving file: " + e);
        }
    }

    private void openStringFile() {
        theString.clear();
        File file = fileChooser.getInputFile(this, "Please select the file to be read");
        if (file == null) {
            return; // user cancelled
        }
        ...
        try {
            ...
            String backgroundRGB = reader.readLine();

            String item;
            while ((item = reader.readLine()) != null) {
                System.out.println(item);
                String[] itemParts = item.split("\\|");
                DrawTextItem drawTextItem = new DrawTextItem(itemParts[0], Integer.parseInt(itemParts[1]), Integer.parseInt(itemParts[2]));
                drawTextItem.setTextColor(new Color(Integer.parseInt(itemParts[3])));

                theString.add(drawTextItem);
                canvas.setBackground(new Color(Integer.parseInt(backgroundRGB)));
                canvas.repaint();
            }

            reader.close();
            JOptionPane.showMessageDialog(canvas, "File loaded, feel free to edit!");

        } catch (Exception e) {
            JOptionPane.showMessageDialog(canvas, "Error reading the file : " + e);
        }
